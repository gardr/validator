...
if (system.args.length <= 1) {
    ...
    var setPath = function (path){
        path = path.split('/');
        return [fs.workingDirectory].concat(path).join(fs.separator);
    };

    // Extended by defaults before injected
    var opt = page.options = JSON.parse(system.args[1]);
    opt.parentUrl = setPath(opt.parentUrl);
    opt.iframeUrl = setPath(opt.iframeUrl);
    opt.managerScriptPath = setPath(opt.managerScriptPath);
    opt.managerInitPath = setPath(opt.managerInitPath);

    opt.scriptUrl = setPath(opt.scriptUrl||opt.fallbackScriptUrl);
