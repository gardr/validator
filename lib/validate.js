...
var KEYS = ['info', 'debug', 'warn', 'error', 'success', 'meta'];
internals.createReportHelper = function (result) {
    return function entryReporter(validatorFileName) {
        ...
        KEYS.forEach(function (key) {
            ...
            reportHelpers[key] = function (message, data) {
                result[key].push(internals.formatTraceData({
                    'message': message,
...
internals.filterDataByDependencies = function (harvested, dependencies, fileName) {
    var res = {
...
internals.preprocess = function (harvested, options, callback) {
    ...
    async.mapSeries(files, function (entry, done) {
        ...
        var filtered = internals.filterDataByDependencies(harvested, mod.dependencies, fileName);
        ...
        try{
            ...
            console.log('Calling preprocess function failed on '+current, e.message/*, e.stack*/);
            done(e);
        }
    ...
    internals.preprocess(harvested, options, function (err) {
        ...
        async.mapSeries(validators, function (entry, done) {
            ...
            try{
                ...
                console.log('Calling validation function failed on '+current, e.message/*, e.stack*/);
                done(e);
            }
