...
internals.createOutputter = function(mod, harvestedData){
    return function outputter(context, key, value) {
        if (typeof value === 'undefined') {
            value = key;
            key   = context;
            context = (mod.output || ( mod.dependencies && mod.dependencies[0] ) || 'common');
        }
        if (!context){
            context = mod.output;
        }

        var missingDefaultOutputTarget = (context !== mod.output &&
            mod.dependencies &&
            mod.dependencies.indexOf(context) === -1);

        if (!harvestedData[context] || missingDefaultOutputTarget) {
            throw new Error('Preprocess output error: You can only output on dependencies or specify output prop. ---> \"'+ context+'\"');
        }

        var target = harvestedData[context];

        if (key.split('.').length > 1){
            key = key.split('.');
            target[key[0]] = target[key[0]]||{};
            target[key[0]][key[1]] = value;
        } else {
            target[key] = value;
        }
    };
...
internals.preprocess = function (harvested, options, callback) {
    ...
    async.mapSeries(files, function (entry, done) {
        ...
        var outputter = internals.createOutputter(mod, filtered);
    ...
    internals.preprocess(harvested, options, function (err) {
        ...
        async.mapSeries(validators, function (entry, done) {
            ...
            try{
                ...
                e.message = 'Validation (\"'+(options.id)+'\") function failed/error on \"'+current+'\". '+e.message;
