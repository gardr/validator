...
module.exports = function (harvested, outputFn, next, options) {
    ...
    */
    function iterator(entry, nextIterator) {
        ...
        function tlsHandler(err, res, body) {
    ...
    function processCheckTls() {
        var someValidTls = Object.keys(rawFileData).some(function(key){return rawFileData[key] && rawFileData[key].validTls;});

        outputFn('validTls', someValidTls);

        var urlsAndCodes = Object.keys(rawFileData).filter(function(key){
        ...
        outputFn('failingUrls', urlsAndCodes);
    }
