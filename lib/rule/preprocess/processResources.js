...
module.exports = function (harvested, outputFn, next, options) {
    ...
    /*
        ...
        4 Tag request error
        5 Check if https available
    */
    function iterator(entry, nextIterator) {

        var url = entry.request.url;

        var isLocalUserEntryFile = url.indexOf(options.resourceDomainBase + '/user-entry.js') === 0;
        var isNotBootstrapFiles = url.indexOf(options.resourceDomainBase) !== 0;
        var isServedZipContent = url.indexOf(options.resourceDomainBase + '/zip') === 0;


        // filter out internals
        var isExternalContent =
            url.indexOf('http') === 0 &&
            (isLocalUserEntryFile || isServedZipContent || isNotBootstrapFiles) &&
        ...
        if (harConfig.checkTls && isLocalUserEntryFile !== true && isServedZipContent !== true) {
            responseCounter += 1;
    ...
    function hasValidTls(){
        ...
        return keys.filter(function(key){
            return typeof rawFileData[key].validTls !== 'undefined' && !rawFileData[key].validTls;
        }).length === 0;
    }

    function getFailingUrls(){
        return Object.keys(rawFileData).filter(function(key){
            return typeof rawFileData[key].validTls !== 'undefined' &&
                          rawFileData[key].validTls !== true;
        }).map(function(key){
            return {
                'url': key,
                'validTls': rawFileData[key].validTls,
