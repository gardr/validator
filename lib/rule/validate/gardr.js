...
internals.validateClick = function (banner, data, windowOpened, report) {
    if (banner.found === false) {
        if (!windowOpened && this.enforceSpec === true) {
            report.error('Window.open not called, might be because banner element is missing.');
        }
        return;
    }
    ...
    if (windowOpened && windowOpened.length > 0) {
        ...
        report.error('Window.open not called, but is enforced by spec.');
    }
...
internals.validateRules = function (harvested, report, next, globalOptions) {
    ...
    internals.validateClick.call(this, gardr.dom.banner, gardr.data, (actions && actions.windowOpened), report);
