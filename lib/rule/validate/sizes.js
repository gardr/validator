...
function getScriptSize(data, regExp) {
    if (!data || !data.typed || !data.typed.types || !data.typed.types.script) {
        return null;
    }

    var size = 0;
    var scripts = data.typed.types.script;
    Object.keys(scripts).forEach(function (scriptUrlKey) {
        if (regExp.test(scriptUrlKey)) {
            size += scripts[scriptUrlKey].bodyLength;
        }
    });
    return size;
}
...
internals.outputAdformThreshold = function (dataSummary, report /*, options*/ ) {

    var size = getScriptSize(dataSummary, RE_ADFORM);
...
var RE_GSAP = /(gsap|tweenlight|tweenmax)/i;
internals.outputGSAPThreshold = function (dataSummary, report /*, options*/ ) {

    var size = getScriptSize(dataSummary, RE_GSAP);

    if (size) {
        report.meta('GSAP', { 'increaseThreshold': size });
    }
};
var RE_JQ = /(jquery)/i;
internals.validate = function (harvested, report, next, globalOptions) {

    report.setChecklist('Sizes', 'Check if resources comes within the limits');
    var baseDesc = 'Base';
    ...
    var summary;
    if (harvested.har.banner && harvested.har.banner.rawFileDataSummary){
        summary = harvested.har.banner.rawFileDataSummary;
    } else {
        summary = harvested.har.all && harvested.har.all.rawFileDataSummary;
    }

    var jQueryFound = !! (harvested.jquery && harvested.jquery.version);
    if (this.giveExtraThreshold.jQuery === true && jQueryFound) {
        var jQuerySize = getScriptSize(summary, RE_JQ);
        report.meta('jQuery', {
            'increaseThreshold': jQuerySize || this.giveExtraThreshold.jQueryThreshold
        });
    }
    ...
    if (summary) {
        ...
        if (this.giveExtraThreshold.GSAP) {
            internals.outputGSAPThreshold.call(this, summary, report, globalOptions);
        }
