...
internals.getSummaryView = function () {
    return [{
        'name': ['Script', 's'],
        'select': 'script'
    }, {
...
internals.outputSummary = function (summary, report) {
    ...
    if (summary.typed && summary.typed.summary) {
        totalSummaryView = internals.getSummaryView()
            .filter(internals.validEntries.bind(null, summary.typed.summary))
            .map(internals.outputEntries.bind(null, summary.typed));
    }
