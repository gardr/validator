function instrumentGardrCollectCSS(config) {
    ...
    if (el !== null) {
        ...
        output.banner.clickHandler = el.onclick && el.onclick.toString();
        output.banner.name = el.tagName;
        output.banner.id = el.id;
        output.banner.html = el.innerHTML;
        output.banner.css  = getCss(window.getComputedStyle(el));
    } else {
        console.log('instrument/gardr.js not found output.banner');
        output.banner.found = false;
        output.banner.html = element.innerHTML;
    }
