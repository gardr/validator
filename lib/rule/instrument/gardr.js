function instrumentGardrCollectCSS(config) {
    ...
    if (!element){
        console.log('!internal instrument/gardr.js DEBUG MISSING ELEMENT instrument gardr.js' + document.body.innerHTML.substring(0, 250) + '');
    }
    ...
    if (config && config.illegalTags){
        output.illegalTags = [config.illegalTags].filter(filterFound).map(report);
    }
    return output;
}
...
function gardrData() {
    ...
    var item = window.__manager._get('!internal_banner_name')[0];
    return {
...
module.exports = {
    'onPageOpen': function onPageOpen(api/*, config*/) {
        ...
        api.trigger('onCustomEvent', {name: 'gardrStart'});
